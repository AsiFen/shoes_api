    ...
    return {
        signup: async (username, password) => {
            try {
                ...
                let existingUser = await db.any('select * from where username = $1', [username])
                ...
                if (existingUser) {
                    ...
                    console.log(newUserId, 'userid for username');
                    //create cart for new user
                    let userCartId = await db.none('insert into cart (user_id) values ($1) RETURNING id', [newUserId])
                    console.log(userCartId, 'user cart id');
                    //return message for new user
                    return { message: 'User created successfully', userId: newUserId, cartid: userCartId };
                }
            } catch (error) {
